<?php
    ...
    Plugin URI: https://www.powr.io/plugins/photo-filter
    Description: Photoshop-style filters for any image!  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    class Powr_Photo_Filter extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Photo Filter.
        </p>
        <p>
    ...
    function powr_photo_filter_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-photo-filter' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-photo-filter' label='$label'></div>";
      }else{
      	"<div class='powr-photo-filter'></div>";
      }
    }
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_photo_filter_menu' );
    function powr_photo_filter_menu() {
      add_menu_page( 'POWr Photo Filter', 'POWr Photo Filter', 'manage_options', 'powr-photo-filter-settings', 'powr_photo_filter_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_photo_filter_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/photo-filter';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/photo-filter';
      }
    ...
    register_activation_hook(__FILE__, 'powr_photo_filter_plugin_activate');
      add_action('admin_init', 'powr_photo_filter_plugin_redirect');

      function powr_photo_filter_plugin_activate() {
      add_option('powr_photo_filter_plugin_do_activation_redirect', true);
      }
      ...
      function powr_photo_filter_plugin_redirect() {
      if (get_option('powr_photo_filter_plugin_do_activation_redirect', false)) {
          delete_option('powr_photo_filter_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-photo-filter-settings&' );
          }
       }
      }
